...
def Sinha(N, total_step, D, d, C, Data, Label):
    x_max = 0.005
    constraint_summary = []
    ...
    for step in range(1, total_step):
        ...
        for n in range(N):
            ...
            xtn[step][n] = ytn[step - 1][n] - 1 / (2 * alpha) * (GD_tnc_f[n] * V * beta + Lambda * np.exp(Lambda * vq[n]) * beta * gtn[n])
...
# Simulation
Loss_Sinha, Cons_Sinha, Acc_Sinha, Ent_Sinha = Sinha(N, total_step, D, d, C, Data, Label)
...
Ent_Sinha_adjusted = adjust_entropy(Ent_Sinha, 7840)

Cons_Sinha = cumulative_average(Cons_Sinha)

plt.figure(figsize=(20, 12))

plt.subplot(2, 2, 1)
plt.plot(Acc_Sinha[1:500], label="[28]", color="blue", linestyle='--', linewidth=2)
...
plt.subplot(2, 2, 2)
plt.plot(Loss_Sinha[1:500], label="[28]", color="blue", linestyle='--', linewidth=2)
plt.xlabel("Time", fontsize=24, fontdict={'fontname': 'Times New Roman'})
...
plt.subplot(2, 2, 3)
plt.plot(Ent_Sinha_adjusted[1:500], label="[28]", color="blue", linestyle='--', linewidth=2)
plt.xlabel("Time", fontsize=24, fontdict={'fontname': 'Times New Roman'})
...
plt.subplot(2, 2, 4)
plt.plot(Cons_Sinha[1:500], label="[28]", color="blue", linestyle='--', linewidth=2)
plt.xlabel("Time", fontsize=24, fontdict={'fontname': 'Times New Roman'})
