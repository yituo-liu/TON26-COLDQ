...
def RECOO(N, total_step, D, d, C, Data, Label):
    x_max = 0.005
    constraint_summary = []
    ...
    for step in range(1, total_step):
        alpha = 1e5
        eta = 1 * np.sqrt(step + 1)
        gamma = 60 * math.pow(step, 1 / 2 + 0.01)
        ...
        for n in range(N):
            ...
            if np.linalg.norm(xtn[step][n] - xtn_quantized[n]) ** 2 > constraint_upper:
                xtn[step][n] = (2 * alpha * ytn[step - 1][n] - GD_tnc_f[n] + 2 * vq[n] * gamma * xtn_quantized[n]) / (2 * alpha + 2 * gamma * vq[n])
                xtn[step][n] = np.clip(xtn[step][n], -x_max, x_max)
        ...
        for n in range(N):
            ...
            xtn_quantized[n], current_ent_x[n] = quantize(xtn[step][n], 3)

            Contraint = max(0, np.linalg.norm(xtn[step][n] - xtn_quantized[n]) ** 2 - constraint_upper)
            time_constraint += Contraint
            gamma_current = 60 * math.pow(step + 1, 1 / 2 + 0.01)
            vq[n] = max(eta, vq[n] + gamma_current * Contraint)

            time_entropy += conditional_entropy_numpy(torch.tensor(last_ent_x[n]), torch.tensor(current_ent_x[n]), 2)
...
# Simulation
Loss_RECOO, Cons_RECOO, Acc_RECOO, Ent_RECOO = RECOO(N, total_step, D, d, C, Data, Label)
...
Ent_RECOO_adjusted = adjust_entropy(Ent_RECOO, 7840)

Cons_RECOO = cumulative_average(Cons_RECOO)

plt.figure(figsize=(20, 12))

plt.subplot(2, 2, 1)
plt.plot(Acc_RECOO[1:500], label="[21]", color="blue", linestyle='--', linewidth=2)
...
plt.subplot(2, 2, 2)
plt.plot(Loss_RECOO[1:500], label="[21]", color="blue", linestyle='--', linewidth=2)
plt.xlabel("Time", fontsize=24, fontdict={'fontname': 'Times New Roman'})
...
plt.subplot(2, 2, 3)
plt.plot(Ent_RECOO_adjusted[1:500], label="[21]", color="blue", linestyle='--', linewidth=2)
plt.xlabel("Time", fontsize=24, fontdict={'fontname': 'Times New Roman'})
...
plt.subplot(2, 2, 4)
plt.plot(Cons_RECOO[1:500], label="[21]", color="blue", linestyle='--', linewidth=2)
plt.xlabel("Time", fontsize=24, fontdict={'fontname': 'Times New Roman'})
