...
def COLDQ(step, choice, choice_aggregation, vq, device):
    ...
    obj = cp.Minimize(
        ...
        + (
            ...
            + vq[2] * (cp.sum(cp.abs(A_estimate[2] @ x - b_estimate[2]) + A_estimate[2] @ x - b_estimate[2]) / 2)
        )
    ...
    for i in range(constraint_dim):
        vq[i] = max(
            gamma,
            (1 - eta) * vq[i] + max(0, np.dot(A_estimate[i], x_new) - b_estimate[i]),
        )
...
# with open('loss_fixedCons_COLDQ.pkl', 'wb') as f:
#     pickle.dump(Avegraged_loss_COLDQ, f)

# with open('violation_fixedCons_COLDQ.pkl', 'wb') as f:
#     pickle.dump(Avegraged_violation_COLDQ, f)

# with open('loss_fixedCons_COLDQ.pkl', 'rb') as f:
#     Avegraged_loss_COLDQ = pickle.load(f)

# with open('violation_fixedCons_COLDQ.pkl', 'rb') as f:
