    ...
    for i in range(N):
        ...
        with torch.no_grad():
            for param_tensor, param in local_models[i].named_parameters():
                if param.requires_grad and param.grad is not None:
                    param.data.copy_(models_agg[i][param_tensor] - param.grad * 0.01)
        ...
        if gradient_norm_sum > constraint_upper:
            with torch.no_grad():
                for param_tensor, param in local_models[i].named_parameters():
                    if param.requires_grad and param.grad is not None:
                        param.data = (models_agg[i][param_tensor] - 0.01 * param.grad
                                        + 0.01 *  2 * vq[i] * gamma * models_quantized[i][param_tensor]) / (1 + 2 * 0.01 * gamma * vq[i])
                        param.data = torch.clamp(param.data, min=-0.1, max=0.1)
...
plt.subplot(1, 2, 1)
plt.plot(plot_accuracy[1:T], label="[27]", color="red", linewidth=2)
...
plt.subplot(1, 2, 2)
plt.plot(Ent_adjusted[1:T], label="[27]", color="red", linewidth=2)
plt.xlabel("Time", fontsize=24, fontdict={'fontname': 'Times New Roman'})
