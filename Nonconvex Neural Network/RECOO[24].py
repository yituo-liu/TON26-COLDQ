...
def RECOO(device, models, criterion, optimizers, weight_matrix, models_quantized, models_agg):
    ...
    pbar = tqdm(range(time_slot), 
                ...
                ncols=100,
                unit='Slot')
    vq = torch.zeros(num_devices).to(device)
    constraint_upper = 1e-4
    for t in pbar:
        running_loss = 0.0
        alpha = 50
        eta = 1 * torch.sqrt(torch.tensor(t + 1).to(device))
        gamma = 0.05 * torch.pow(torch.tensor(t).to(device), 1 / 2 + 0.01)
        time_accuracy = 0
                    ...
                    for param_tensor, param in models[i].named_parameters():
                        if param.requires_grad and  param.grad is not None:
                            param.data = (2 * alpha * models_agg[i].state_dict()[param_tensor] - param.grad
                                           + 2 * vq[i] * gamma * models_quantized[i].state_dict()[param_tensor]) / (2 * alpha + gamma * 2 * vq[i])
                            param.data = torch.clamp(param.data, min=-0.1, max=0.1)
        ...
        for i in range(len(models)):
            ...
            gamma_current = 0.05 * torch.pow(torch.tensor(t + 1).to(device), 1 / 2 + 0.01)
            vq[i] = torch.max(eta, vq[i] + gamma_current * Constraint)
        ...
        if (t + 1) % 1 == 0:
            pbar.write(f'RECOO {t+1}, Loss: {time_loss:.2f}, Acc: {plot_accuracy[t]:.2f}%, Cons: {constraint_summary[t]:.2f}, Ent: {Entropy[t]:.4f}')
...
# Training
Loss_RECOO, Constraint_RECOO, Acc_RECOO, Ent_RECOO = RECOO(device, models, criterion, optimizers, weight_matrix, models_quantized, models_agg)


Acc_RECOO = Acc_RECOO.cpu().clone().detach()
Ent_RECOO = torch.tensor(Ent_RECOO).cpu()
...
Acc_RECOO = cumulative_average(Acc_RECOO.numpy())
Ent_RECOO_adjusted = adjust_entropy(Ent_RECOO.numpy(), 40970)


plt.figure(figsize=(16, 8))

plt.subplot(1, 2, 1)
plt.plot(Acc_RECOO[1:time_slot], label="[24]", color="blue", linestyle='--', linewidth=2)
...
plt.subplot(1, 2, 2)
plt.plot(Ent_RECOO_adjusted[1:time_slot], label="[24]", color="blue", linestyle='--', linewidth=2)
plt.xlabel("Time", fontsize=24, fontdict={'fontname': 'Times New Roman'})
...
# Save results
# with open('Nonconvex_RECOO_Acc.pkl', 'wb') as f:
#     pickle.dump(Acc_RECOO, f)

# with open('Nonconvex_RECOO_Ent.pkl', 'wb') as f:
#     pickle.dump(Ent_RECOO, f)
