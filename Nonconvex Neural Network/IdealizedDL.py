...
param_max = torch.tensor(0.0, device=device)
        ...
        with torch.no_grad():
            for param_tensor, param in local_models[i].named_parameters():
                if param.requires_grad and param.grad is not None:
                    ...
                    param_max = torch.max(param_max, torch.max(torch.abs(param.data)))
...
plt.plot(plot_accuracy[1:T], label="Idealized DL", color="red", linewidth=2)
