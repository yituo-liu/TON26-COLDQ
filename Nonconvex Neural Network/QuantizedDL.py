...
def QuantizedDL(device, models, criterion, optimizers, weight_matrix, models_quantized, models_agg):
    ...
    for t in pbar:
        ...
        if (t + 1) % 1 == 0:
            pbar.write(f'QuantizedDL {t+1}, Loss: {time_loss:.2f}, Acc: {plot_accuracy[t]:.2f}%, Cons: {constraint_summary[t]:.2f}, Ent: {Entropy[t]:.4f}')
...
# Training
Loss_QuantizedDL, Constraint_QuantizedDL, Acc_QuantizedDL, Ent_QuantizedDL = QuantizedDL(device, models, criterion, optimizers, weight_matrix, models_quantized, models_agg)


Acc_QuantizedDL = Acc_QuantizedDL.cpu().clone().detach()
Ent_QuantizedDL = torch.tensor(Ent_QuantizedDL).cpu()
...
Acc_QuantizedDL = cumulative_average(Acc_QuantizedDL.numpy())
Ent_QuantizedDL_adjusted = adjust_entropy(Ent_QuantizedDL.numpy(), 40970)
...
plt.subplot(1, 2, 1)
plt.plot(Acc_QuantizedDL[1:time_slot], label="QuantizedDL", color="orange", linestyle='-.', linewidth=2)
...
plt.subplot(1, 2, 2)
plt.plot(Ent_QuantizedDL_adjusted[1:time_slot], label="QuantizedDL", color="orange", linestyle='-.', linewidth=2)
plt.xlabel("Time", fontsize=24, fontdict={'fontname': 'Times New Roman'})
...
# Save results
# with open('Nonconvex_QuantizedDL_Acc.pkl', 'wb') as f:
#     pickle.dump(Acc_QuantizedDL, f)

# with open('Nonconvex_QuantizedDL_Ent.pkl', 'wb') as f:
#     pickle.dump(Ent_QuantizedDL, f)
